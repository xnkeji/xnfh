...
<body class="bg-gradient-to-br from-blue-50 via-white to-blue-100 min-h-screen flex items-center justify-center px-2 py-6 font-sans text-dark">
  <div class="container max-w-md w-full bg-white/90 rounded-3xl card-shadow transition-all duration-300 hover:card-shadow-hover overflow-hidden backdrop-blur-2xl">
    ...
    <div class="text-center pt-2 pb-1 px-5">
      <h1 class="text-2xl md:text-2xl font-extrabold text-primary drop-shadow-sm tracking-wide mb-2">小年防红链接生成器</h1>
      <p class="text-gray-500 text-sm mb-2">安全转换 · 稳定防红 · 付费使用</p>
    </div>
    ...
    <div class="p-4 pt-2 mx-3">
      <div class="mt-4">
        ...
        <select id="apiSelector" class="w-full pl-2 pr-4 py-3 rounded-lg border border-gray-300 focus:outline-none focus:ring-2 focus:ring-primary transition-all duration-200 text-sm bg-gray-50">
          <option value="https://xiaoniankeji.github.io/xnkjfh/xnfh.html?c=" selected>默认接口</option>
          <option value="https://xnj886.github.io/hg1/1.html?c=">备用接口1</option>
          <option value="https://yet-another-api-url.com?c=">备用接口2</option>
        </select>
      </div>

      <label for="targetUrl" class="block text-gray-700 font-medium mb-2 text-sm mt-4">
      ...
      <div class="mt-4">
        ...
        <div class="flex flex-wrap gap-2">
          <input type="radio" name="qrcolor" id="qrcolor1" class="hidden color-radio" value="#000000" checked>
          <label for="qrcolor1" class="w-7 h-7 rounded-full border-2 border-gray-300 flex items-center justify-center cursor-pointer" style="background:#000000"></label>
          ...
          <input type="radio" name="qrcolor" id="qrcolor5" class="hidden color-radio" value="#2563eb">
          <label for="qrcolor5" class="w-7 h-7 rounded-full border-2 border-gray-300 flex items-center justify-center cursor-pointer" style="background:#2563eb"></label>
        </div>
      </div>
    ...
    <div class="p-4 mt-4 border-t border-gray-100 text-center text-xs text-gray-400">
      <p>
        搭建同款 | <span class="text-primary cursor-pointer underline" onclick="showWechat()">QQ扫码咨询</span>
      </p>
      <div id="wechatModal" class="hidden fixed inset-0 bg-black/40 z-50 flex items-center justify-center">
        <div class="bg-white p-4 rounded-2xl shadow-lg max-w-xs w-full relative animate-fadeIn">
          ...
          <img src="qq.png" alt="微信二维码" class="w-full rounded-xl mb-2">
